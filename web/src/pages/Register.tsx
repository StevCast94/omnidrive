...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // El registro ya devuelve la sesion: no hace falta un login extra ni
      // una tercera llamada para el perfil.
      ...
      <div className="w-full max-w-lg">
        ...
        <div className="text-center mb-8">
          <button onClick={() => navigate('/')} className="inline-flex flex-col items-center gap-1.5 text-2xl font-bold text-white mb-2 cursor-pointer hover:opacity-80 transition-opacity">
            {/* El ancho va en este contenedor, no en LogoMark: puesto en el
                propio <LogoMark>, compite con el 'w-full' que ya trae el SVG
                por dentro, y gana el que haya quedado despues en la hoja de
                estilos generada — no el que se ve al final en el codigo. Eso
                dejaba el isotipo enorme de forma inconsistente. */}
        ...
        <form onSubmit={handleSubmit} className="bg-slate-900 rounded-2xl p-8 border border-slate-800 space-y-5">
          ...
          <div className="flex justify-center">
            <GoogleButton onToken={entrarConGoogle} texto="signup_with" disabled={loading} />
          </div>
        </form>
