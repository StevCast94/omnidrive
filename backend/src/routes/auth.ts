...
authRouter.post('/oauth-profile', asyncHandler(async (req: Request, res: Response) => {
...
authRouter.post('/verificar-cedula', authenticate, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  if (banned) return res.status(403).json({ data: null, error: 'Esta cedula ha sido vetada en la plataforma.', code: 'BANNED_IDENTITY' });
...
authRouter.post('/verificar-whatsapp', authenticate, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  if (!phone) return res.status(400).json({ data: null, error: 'Numero de telefono requerido' });
...
authRouter.post('/avatar', authenticate, avatarUpload.single('avatar'), asyncHandler(async (req: AuthRequest, res: Response) => {
  if (!req.file) return res.status(400).json({ data: null, error: 'No se envio ninguna imagen' });
